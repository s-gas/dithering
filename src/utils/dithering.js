...
const dither = (ctx, width, height) => {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const closestColor = findClosestColor(rgbArray[i], rgbArray[i + 1], rgbArray[i + 2], palette);

      rgbArray[i] = closestColor[0];
      rgbArray[i + 1] = closestColor[1];
      rgbArray[i + 2] = closestColor[2];
    }
  }
